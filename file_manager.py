...
# ──────────────────────────────────────────────────────────────
#  SCROLL MANAGER  — macOS universal yechim
#  macOS da Canvas ichidagi Frame'lar MouseWheel'ni ushlamaydi,
#  shuning uchun barcha widget uchun bind_all ishlatamiz,
#  lekin faqat cursor shu canvas ustida bo'lganda scroll qilamiz.
# ──────────────────────────────────────────────────────────────
class ScrollManager:
    """
    macOS da tkinter'da scroll ishlatishning yagona ishonchli usuli:
    bind_all bilan global event ushlab, cursor qaysi canvas ustida
    ekanini tekshirib, o'sha canvasni scroll qilish.
    """
    def __init__(self):
        self._canvases = []   # (canvas, check_fn) ro'yxati

    def register(self, canvas):
        """Canvas'ni ro'yxatga qo'sh."""
        self._canvases.append(canvas)
        return canvas

    def setup(self, root):
        """Root oynaga global scroll binding."""
        if IS_MAC:
            root.bind_all("<MouseWheel>", self._on_scroll_mac, add="+")
        else:
            root.bind_all("<Button-4>",   self._on_scroll_linux, add="+")
            root.bind_all("<Button-5>",   self._on_scroll_linux, add="+")
            root.bind_all("<MouseWheel>", self._on_scroll_win,   add="+")

    def _get_canvas_under_cursor(self, event):
        """Cursor ostidagi ro'yxatdagi canvasni qaytaradi."""
        wx, wy = event.widget.winfo_rootx(), event.widget.winfo_rooty()
        # event koordinatalari screen koordinatalarida
        sx = wx + event.x
        sy = wy + event.y
    ...
    def _on_scroll_mac(self, event):
        cv = self._get_canvas_under_cursor(event)
        if cv is None:
            return
        # macOS: delta kichik, lekin birlik — to'g'ridan-to'g'ri ishlatish
        # Trackpad bir harakatda delta=1..5 beradi
...
# ──────────────────────────────────────────────────────────────
#  RIGHT-CLICK MANAGER  — macOS universal yechim
#  macOS da Button-3 ishlashi uchun Tk'ni maxsus sozlash kerak,
#  yoki Button-2 + Control-Button-1 bind qilish kerak.
# ──────────────────────────────────────────────────────────────
def bind_rc(widget, callback):
    """
    Right-click uchun universal binding.
    - Button-3 : barcha platformada asosiy
    - Button-2 : macOS'da ba'zi sichqoncha/trackpad konfiguratsiyalari
    - Control-Button-1 : macOS'da Ctrl+klik = right-click
    """
    if IS_MAC:
...
def show_menu(menu, event):
    """
    Context menu'ni xavfsiz ko'rsatish.
    macOS da grab_release xato beradi — try/except bilan himoyalangan.
    """
    try:
...
class FileRow(tk.Frame):
    def __init__(self, master, item, idx, on_sel, on_open, on_right, **kw):
        ...
        name_font = ("Consolas", 10, "italic") if dim else ("Consolas", 10)
        self._nm = tk.Label(self, text=("· " if dim else "") + item["name"],
                            bg=bg, fg=name_fg, font=name_font, anchor="w", width=34)
        ...
        self._ws = [self, self._bar, self._ic, self._nm,
                    self._sz, self._ty, self._dt]
        ...
        for w in self._ws:
            ...
            bind_rc(w, lambda e, i=item: on_right(e, i))
    ...
    def select(self, yes):
        ...
        if yes:
            self._sbg(C["row_sel"])
            self._bar.configure(bg=C["row_sel_bd"])
...
class FileManager(ctk.CTk):
    def __init__(self):
        ...
        self._ui()
        # Global scroll setup — barcha canvaslar register qilingandan keyin
        SCROLL.setup(self)
    ...
    def _sidebar(self, p):
        ...
        self._tcv.pack(fill="both", expand=True)
    ...
    def _rightpanel(self, p):
        ...
        self._pf.pack(fill="x", padx=14)
        self._pf.pack_propagate(False)
        ...
        cols = [
            ...
            ("Hajm",         "size",     10, "e"),
            ("Fayl turi",    "type",     15, "w"),
            ("Ozgartirilgan","modified", 18, "w"),
        ]
        for lbl, cid, w, anc in cols:
            clr = C["cyan"] if (cid and cid == self._scol) else C["text3"]
            txt = lbl + ("  " + ("▼" if self._srev else "▲")
                         if cid and cid == self._scol else "")
        ...
        # Register main list canvas
        SCROLL.register(self._cv)
    ...
    def _back(self):
        if self._hi > 0:
            self._hi -= 1
            self._load(self._hist[self._hi], push=False)
    ...
    def _draw(self, items):
        ...
        self._selrow = None
        if not items:
            self._empty("📭  Hech narsa topilmadi")
            return
        for i, it in enumerate(items):
            FileRow(self._ff, it, i,
                    on_sel=self._selrow_fn,
                    on_open=self._open,
                    on_right=self._ctx).pack(fill="x")
    ...
    def _tick(self):
        ...
        try:
            ...
            pass
        if batch:
            self._gres.extend(batch)
            self._draw(self._gres)
    ...
    def _open(self, item):
        if item["is_dir"]:
            self._load(item["entry"].path)
        else:
            import subprocess
            try:
                if sys.platform == "win32":
                    os.startfile(item["entry"].path)
                elif sys.platform == "darwin":
                    subprocess.Popen(["open", item["entry"].path])
                else:
                    subprocess.Popen(["xdg-open", item["entry"].path])
            except Exception as ex:
                messagebox.showerror("Xato", str(ex))
    ...
    def _paste(self):
        ...
        if os.path.exists(dst):
            ...
            dst = os.path.join(self._cur, f"{b}_nusxa{e}")
        try:
    ...
    def _delete(self, item):
        if messagebox.askyesno(
            ...
            try:
                ...
                self._setstatus(f"🗑️  Ochirildi: {item['name']}")
                self._refresh()
            except Exception as ex:
                messagebox.showerror("Xato", str(ex))
    ...
    def _newfolder(self):
        ...
        if nm:
            try:
                ...
                self._setstatus(f"✅  Yaratildi: {nm}")
                self._refresh()
            except Exception as ex:
                messagebox.showerror("Xato", str(ex))
    ...
    def _props(self, item):
        ...
        rows = [
            ...
            ("📅 Yaratilgan",
             datetime.datetime.fromtimestamp(st.st_ctime).strftime("%Y-%m-%d  %H:%M")
             if st else "—"),
